...
class GFG {
	...
	static void J_140printKPathUtil(Node root, int k)
	{
		if (root == null)
			return;
		...
		J_140printKPathUtil(root.left, k);

		// check if there's any k sum path
		// in the right sub-tree.
		J_140printKPathUtil(root.right, k);
	...
	// A wrapper over J_140printKPathUtil()
	static void printKPath(Node root, int k)
	{
		path = new Vector<Integer>();
		J_140printKPathUtil(root, k);
	}
